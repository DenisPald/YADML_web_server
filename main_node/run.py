...
async def execute_remote_training(ip: str, ssh_config: dict, script_path: str, args: dict[str, str]):
    cmd = f"python3 {script_path} " + " ".join([f"--{k} {v}" for k, v in args.items()])
    print(f"Запуск команды на {ip}:{ssh_config["port"]} : {cmd}")

    async with asyncssh.connect(ip, **ssh_config) as conn:
...
def collect_remote_models(nodes: dict[str, dict], remote_model_dir: str, remote_model_name:str, local_model_dir: str) -> list[str]:
    os.makedirs(local_model_dir, exist_ok=True)
    local_model_paths = []

    for ip, config in nodes.items():
        remote_model_path = f"{remote_model_dir}/{remote_model_name}"
        local_model_path = os.path.join(local_model_dir, f"model_{ip.replace('.', '_')}_{config['port']}.pt")
...
async def load_nodes_config(config_path="conf.json"):
    """
    ...
    # Преобразуем список в формат словаря с ключами по имени узлов
...
async def main():
    ...
    for global_epoch in range(global_epochs):
        ...
        for ip, config in nodes.items():
            args = {
                "data_path": f"{remote_dir}/split_{list(nodes.keys()).index(ip)}.pt",
                "epochs": "5",  # Количество локальных эпох на узле
                "batch_size": "32",
                "model_save_path": f"{remote_model_dir}/model_{global_epoch}.pt"
            }
            tasks.append(execute_remote_training(ip, config, remote_script_path, args))
